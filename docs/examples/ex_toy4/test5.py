...
# ex_toy4
#
# This is used to make some plots of the TEST data
# using the most recent model saved by the train5.py
# script.
#
...
#------------ Read in TEST samples and infer

# Image generator reads input images from file
...
print ('Looping through test events with model from epoch%03d ...' % epoch_loaded)
...
bins_def_z   = np.linspace(-2.0, 2.0, 201)

# First, define the figure which consists of four plots:
#     top = resolution histograms (prediction-truth)
#  bottom = scatter plots (prediction vs. truth)
#    left = phi
#   right = z
...
ax3.set_facecolor((0.9,0.9,0.99))
ax4.set_facecolor((0.9,0.9,0.99))

# Create histograms and save bin contents and definitions for fitting later
n_phi, bins_phi, p_phi = ax1.hist(diff_phi, bins_def_phi, alpha=0.5, label='phi', color='red')
n_z  , bins_z  , p_z   = ax2.hist(diff_z  , bins_def_z  , alpha=0.5, label='z'  , color='blue')
...
# Scatter plots
ax3.scatter(phi_truth, phi_model, marker='.', alpha=0.5, label='phi weighted avg.', color='red')
ax4.scatter(  z_truth,   z_model, marker='.', alpha=0.5, label='z weighted avg.', color='blue')

ax3.legend(loc='upper left')
ax3.set_title('Correlation after %d epochs' % epoch_loaded)
ax3.set_xlabel('phi truth (cm)')
ax3.set_ylabel('phi model (cm)')
ax4.legend(loc='upper left')
ax4.set_title('Correlation after %d epochs' % epoch_loaded)
ax4.set_xlabel('z truth (cm)')
ax4.set_ylabel('z model (cm)')
...
if fit_phi_success: ax1.text(0.10, 0.70*popt_phi[0], '$\sigma_{\phi}$   = %5.4f$^o$' %  popt_phi[2], fontsize=15)
if fit_z_success  : ax2.text(0.10, 0.55*popt_z[0], '$\sigma_{z}$ = %5.4f cm' %  popt_z[2], fontsize=15)
